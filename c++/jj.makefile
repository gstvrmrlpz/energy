...
data:
	@make -f jj.makefile -s exp | tee $${HOSTNAME%%.*}-$$(date '+%Y%m%d-%H%M%S').$(OUT)
...
exp: pre
	...
	for compiler in $(COM); do
		for opt in $(OPT); do
			for std in $(STD); do
				for exe in $(EXE); do
					make -f jj.makefile -Bs $$exe XTR=$$opt XT2=$$std
					for size in $(SIZ); do
						for work in $(WRK); do
...
quick:
	@make -f jj.makefile -s data OUT=txt REP=1 SIZ=1
