...
pccito.std.raw <- read.csv("../data/v3/pccito-20240726-143415.csv",sep=";")
...
pccito.std.noinit <- pccito.std.raw[ pccito.std.raw$work == "g" | pccito.std.raw =="h",]

pccito.std.noinit.diff <- data.frame(joules=numeric(),time=numeric())
for ( i in pccito.std.noinit ) {
}
@
