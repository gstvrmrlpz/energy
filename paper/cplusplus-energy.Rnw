...
wilcox.packed <- data.frame(length=numeric(),compiler=character(),opt=character(),std=character(),p.value=numeric(),significant=logical())
wilcox.unpacked <- data.frame(length=numeric(),compiler=character(),opt=character(),std=character(),p.value=numeric(),significant=logical())
  ...
  for (compiler in c("clang++","g++")) {
    for (opt in c("-O2","-O3")) {
      for ( std in c("-std=c++17","-std=c++20","-std=c++23")) {
        ...
        wilcox.packed <- rbind(wilcox.packed,data.frame(length=l,compiler=compiler,opt=opt,std=std,p.value=wt$p.value,significant=wt$p.value < 0.05))
        wt.u <- wilcox.test(
        ...
        wilcox.unpacked <- rbind(wilcox.unpacked,data.frame(length=l,compiler=compiler,opt=opt,std=std,p.value=wt.u$p.value,significant=wt.u$p.value < 0.05))
      }
    }
  }
}
