...
\affiliation[citic]{organization={CITIC},%Department and Organization
            city={Granada},
            postcode={18071},
            country={Spain}}
...
The best practice used so far is to take into account only integer-based operations: crossover and mutation (which are called many times in an evolutionary algorithm) and an integer based fitness function. While MaxOnes \cite{back1993optimal} is popular and used extensively in benchmarking new operators and algorithm versions, computing requires a relatively low amount of energy, so it can be considered alongside mutation and crossover \cite{zig1,zig2}; other more complicated fitness functions can also be taken into account: HIFF \cite{watson1998modeling} is one of them, with the added advantage of being a recursive function, an operation that involves heap and stack operations and that can reveal energy inefficiencies in the language it is used \cite{zig1,zig2}.
...
C++ is a compiled language \cite{godbolt2020optimizations}. Compiling consists in examining statically the code and performing a series on transformation on the parsed version so that the machine code produced is as efficient as possible.
...
\item Code generation: target machine code for an operationg system is finally generated.
\end{enumerate}
...
\bibliography{cplusplus-energy,energy,ours,libraries,GAs,cplusplus}
