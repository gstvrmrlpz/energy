...
\item The second level is the different options that you can use when generating the executable. \begin{itemize}
    ...
    %\item Some compiler use an intermediate representation, mainly with the object of allowing static analysis as well as cross-compilation, but this representation can have an impact optimization as well as energy consumption. % no es posible probarlas
    %\item Finally, compilers allow user selection of the language standard that is being followed. This might affect, besides the language constructs that are allowed, how code is generated. % no afecta significativamente
\end{itemize}
...
There is no fixed methodology for measuring energy consumption across a number of different machines with possibly different operating systems and hardware architectures. In Jay el al. \cite{jay2023experimental} we can delve into energy comsumption, it's mesurement and software tools needed to gather information about. To summarise, the most widely used for its convenience is the Running Average Power Limit (RAPL) \cite{david2010rapl} interface through a wide range of tools like perf \cite{de2010new}.

This is the approach followed in previous work \cite{zig1,zig2} but using the pinpount \cite{9307947} software as perf alternative for Mac OS:
\begin{itemize}
\item Choose a tool for energy profiling. This assumes that, in advance it has been already decided which functions were the target for profiling, since energy profilers are unable to measure energy consumption at the function, or function call, level, thus needing to create executables where single functions are executed. To reduce the number of options, and been able to compare with previos cited work, we settled for 3 divisions on the algorithms to be measured:
    \begin{itemize}
    \item Initialization of the population.
    \item Complete Simple Genetic Algorithm cicle with mutation, crossover and onemax evaluation.
    \item HIFF evaluation.
    \end{itemize}
...
# generate a table with kable for pccito.init that uses as first sort criteria the program, then the length, then the optimization level, then the compiler
...
pccito.summary[ pccito.summary$opt=="-O3",] %>% arrange(length,opt,X.compiler,program) %>% kable(caption="Energy consumption by compiler and data structure, optimization level = O3 \\protect\\label{tab:energy.setup.2}") %>% kable_styling(bootstrap_options = c("striped"))
...
\item \texttt{string} uses C++ strings, with 1 and 0 represented as the equivalent character.
\end{itemize}
