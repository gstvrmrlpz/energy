...
\item The first level is the compiler choice one: Most compiled languages have different compilers, designed and produced by different open source or commercial organizations. In the case of C++, there are at least two major ones, clang++ and g++ (from GNU) % Need to add citations here
