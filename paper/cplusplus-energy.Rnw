...
There is no fixed methodology for measuring energy consumption across a number of different machines with (possibly) different operating systems and hardware architectures. A straightforward approach, used in \cite{zig1,zig2} is the following:\begin{itemize}
...
Modern processors provide several ways to measure energy consumption. Most are based in the next standard interfaces:
\begin{itemize}
\item Performance Application Programming Interface (PAPI) \cite{doi:10.1177/109434200001400303} is an interface to hardware and software performance counters on modern microprocessors. Extensive information available in its web page \url{https://icl.utk.edu/papi/}.
\item Running Average Power Limit (RAPL) is an interface for reporting the accumulated energy compsumption of power domains.
\end{itemize}
...
\bibliography{cpluplus-energy,ours}
