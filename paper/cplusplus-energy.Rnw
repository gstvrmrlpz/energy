...
There is no fixed methodology for measuring energy consumption across a number of different machines with possibly different operating systems and hardware architectures. \cite{jay2023experimental} delves into energy consumption, its measurement and the software tools needed to gather information about. To summarise, % summarise no existe, y además en un artículo de revista no se puede resumir. Habría que hacer una revisión extensa de lo que hay y entre lo que hay, qué se ha escogido.
the most widely used for its convenience is the Running Average Power Limit (RAPL) \cite{david2010rapl} interface through a wide range of tools like {\sf perf} \cite{de2010new}.
