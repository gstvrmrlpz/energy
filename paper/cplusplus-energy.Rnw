...
pccito.energy.lm <- lm(energy ~  program + opt + compiler + length, data=pccito.init)
...
amd.energy.lm <- lm(energy ~  program + opt + compiler + length, data=amd.init) # saved for later
